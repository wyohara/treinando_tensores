import pytest
from pathlib import Path
import json
from src.tokenizador.processadores_texto.arvore_trie import ArvoreTrie

@pytest.fixture(scope='function')
def fixture_arquivo_arvore_json(tmp_path):        
    # Cria diretório temporário para dataset
    arvore_json = tmp_path / "pasta_test"
    try:
        arvore_json.mkdir()
    except FileExistsError:
        pass
    yield arvore_json / 'arvore.json'

@pytest.fixture(scope='function')
def fixture_arquivo_css_tokens(tmp_path):        
    # Cria diretório temporário para dataset
    arvore_json = tmp_path / "pasta_test"
    try:
        arvore_json.mkdir()
    except FileExistsError:
        pass
    yield arvore_json / 'arquivo_css_tokens.css'

@pytest.fixture(scope='function')
def classe_arvore_trie_testavel(fixture_arquivo_arvore_json, fixture_arquivo_css_tokens):
    trie = ArvoreTrie()
    trie.set_arquivo_csv_tokens(fixture_arquivo_css_tokens)
    trie.set_arquivo_json_arvore(fixture_arquivo_arvore_json)
    yield trie

@pytest.fixture(scope='function')
def fixture_json_arvore_trie():
    yield ['amor amar amado', {'20': {'fim': 2},'61': {'6d': {'61': {'64': {'6f': {'fim': 1}}, '72': {'fim': 1}, 'fim': 2}, '6f': {'72': {'fim': 1}}, 'fim': 3}}}]



#=============================================================
#               Classe de teste
#=============================================================
class TestArvoreTrie:        
    @pytest.fixture(scope='class', autouse=True)
    def setup_inical(self, request):
        #rodando os testes
        print(f'\n🚀 Rodando o teste...')
        yield

        #teardown
        print(f'\n🧹 rodando o teardown')
    
    def test_processar_texto_usando_trie(self,classe_arvore_trie_testavel, fixture_json_arvore_trie):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        resultado = trie.processar_textos(fixture_json_arvore_trie[0])
        assert resultado == fixture_json_arvore_trie[1]

    def test_salvar_arvore_trie_arquivo_nao_existe(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        assert trie.salvar_arvore_trie({'oi':1}) == True
        assert trie.get_arquivo_json_arvore.exists() == True
    
    def test_salvar_arvore_trie_arquivo_vazio(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        assert trie.salvar_arvore_trie({}) == False

    def test_carregar_arvore_trie_vazia(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        with trie.get_arquivo_json_arvore.open('w', encoding='utf-8') as arq:
            arq.write('')
        assert trie.get_arvore_trie() == {}

    def test_carregar_arvore_trie_vazia_caso2(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        with trie.get_arquivo_json_arvore.open('w', encoding='utf-8') as arq:
            arq.write('{}')
        assert trie.get_arvore_trie() == {}
    
    def test_carregar_arvore_trie_existe(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        trie.salvar_arvore_trie({'oi': 1})
        assert trie.get_arvore_trie() == {'oi': 1}
    
    def test_salvar_arvore_trie_existente(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel     
        assert trie.salvar_arvore_trie({'oi': 1}) == True
        assert trie.salvar_arvore_trie({'casa': 'minha'}) == True
        assert trie.get_arvore_trie() == {'casa': 'minha'}
    
    def test_gerar_lista_tokens(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        assert len(trie.montar_lista_tokens())>0
    
    def test_gerar_lista_tokens(self,classe_arvore_trie_testavel, fixture_json_arvore_trie):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        trie.processar_textos(fixture_json_arvore_trie[0])
        opcional = []
        for i in trie.montar_lista_tokens():
            if i[2]=='opcional': 
                opcional.append(i[2])
        assert len(opcional) == 6 #['am', 'a', 'r', 'o', 'do', ' '] em hex
        assert len(trie.montar_lista_tokens())>5

    def test_gerar_lista_tokens(self,classe_arvore_trie_testavel , fixture_json_arvore_trie, fixture_arquivo_arvore_json):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        with fixture_arquivo_arvore_json.open('w',encoding='utf-8') as arquivo:
            json.dump(fixture_json_arvore_trie[1], arquivo, ensure_ascii=False, separators=(',', ':'))
        opcional = []
        for i in trie.montar_lista_tokens():
            if i[2]=='opcional': 
                opcional.append(i[2])
        assert len(opcional) == 6 #['am', 'a', 'r', 'o', 'do', ' '] em hex
        assert len(trie.montar_lista_tokens())>5

    def test_verificar_se_cria_arquivo_tokens(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        resultado = trie.salvar_csv_tokens([])
        assert resultado == False

    def test_verificar_se_salva_lista_tokens(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        resultado = trie.salvar_csv_tokens([['a'],[1],['utf-8']])
        assert resultado == True
    
    def test_abrir_arquivo_vazio(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        trie.salvar_csv_tokens([])
        assert trie.carregar_lista_tokens() == []
        
    def test_abrir_arquivo_sem_dados(self, classe_arvore_trie_testavel):
        trie:ArvoreTrie = classe_arvore_trie_testavel
        trie.salvar_csv_tokens([])
        assert trie.carregar_lista_tokens() == []
    
    def test_processo_completo(self,classe_arvore_trie_testavel, fixture_json_arvore_trie):        
        trie:ArvoreTrie = classe_arvore_trie_testavel
        #verifica se o primeiro texto foi processado
        processado = trie.processar_textos(fixture_json_arvore_trie[0])
        assert processado == fixture_json_arvore_trie[1]
        assert trie.get_arquivo_json_arvore.is_file() == True

        #verifica se o segundo texto foi processado
        processado = trie.processar_textos(fixture_json_arvore_trie[0])
        assert processado == {'20': {'fim': 4},'61': {'6d': {'61': {'64': {'6f': {'fim': 2}}, '72': {'fim': 2}, 'fim': 4}, '6f': {'72': {'fim': 2}}, 'fim': 6}}}
        assert trie.get_arquivo_json_arvore.is_file() == True

        #criando o csv de tokens
        tokens = trie.montar_lista_tokens()
        salvo = trie.salvar_csv_tokens(tokens)
        assert salvo == True
        assert trie.get_arquivo_csv_tokens.is_file() == True
        lista_tokens =  trie.carregar_lista_tokens()
        assert lista_tokens != []
